...
LatLonItem::LatLonItem() {
	...
	qDebug()<<"LatLonItem constructor..";
	this->convert = &LatLonConvert::getInstance();
	isShow = false;
}
...
{
	//qDebug()<<"[LatLonItem - drawLatLon] called.. mpp : "<<convert->mpp();
...
void LatLonItem::drawLonLines(QPainter* painter)
{
	...
	// 경도 그리기
	double wj = convert->greenich();				//그리니치 포인트 반환

	double dw = convert->baseWJ(wj, dy);
	//qDebug()<<"[LatLonItem - drawLonLines] dy : "<<dy<<", greenich : "<<wj<<", dw : "<<dw<<", mpp :"<<convert->mpp();
	double lon = dw + dy;
...
bool LatLonItem::drawLine3(QPainter *painter, QVector<QPoint>& dataxy)
{
	...
	if (data2.count() > 1) {
		...
		//포물선들은 하나하나의 점으로 연결된다..
		for (int i=0; i<data2.count(); i++) {
			latPoints[i] = data2[i];
			//qDebug()<<"Point : "<<latPoints[i];
		}
...
void LatLonItem::drawText1(QPainter *painter, const QPoint& p, const QString& text)
{
	...
	textBox.setRect(p.x(), p.y(), 20*text2.length(), 25);
