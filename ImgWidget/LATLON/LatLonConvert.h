...
public:
	...
	static LatLonConvert &getInstance()
	{
		if (pinstance == NULL) {
			pinstance = new LatLonConvert;
		}
		return *pinstance;
	}
	...
	double mpp2() { return meterPerPixel_2; }	//added
	...
	void setMeterPerPixel_2(double mpp) { meterPerPixel_2	 = mpp; }		//added
	...
	//added by jjw 20130729
	QString decimalLatToString(double lat);
	QString decimalLonToString(double lon);
	double distanceXY(QPoint p1, QPoint p2);
...
private:
	...
	static LatLonConvert * pinstance;
	...
	double meterPerPixel_2;	//위경도 그리드에 사용
};
