...
double CalcAir::strToDD(const QString& dms)
{
	...
	//qDebug()<<"[strToDD] deg : "<<deg<<", min : "<<min<<", sec : "<<sec;
...
{
	//qDebug()<<"[CalcAir - LatltonToCartesian] wj : "<<wonJum<<", point : "<<p;
	QPointF p1 = QPointF(deg2rad * (p.x()), deg2rad * p.y());

	//test
	//this->LatLonToCartesianTest( deg2rad * wonJum.x(),  deg2rad * wonJum.y());
	QPointF p2 = QPointF(p.x(), p.y() + wonJum.y());
	qDebug()<<"p2 : "<<p2;
	this->LatLonToCartesianTest( p1.x(), p1.y());
	...
	qDebug()<<"LATLON : "<<p1<<"Cartesian X : "<<QString().setNum(cartesian->x, 'f', 6)<<", Y : "<<QString().setNum(cartesian->y, 'f', 6)
			<<", Z : "<<QString().setNum(z, 'f', 6);

	double x = cartesian->x;
	double r1 = sqrt(z * z + x * x);						//반지름?
	//qDebug()<<"R1 : "<<QString().setNum(r1,'f',8);
	double beta;
	...
	/*qDebug()<<"Cartesian X : "<<QString().setNum(cartesian->x, 'f', 6)<<", Y : "<<QString().setNum(cartesian->y, 'f', 6)
			<<", Z : "<<QString().setNum(cartesian->z, 'f', 6);*/

	return;
}

/**
 * 해당 함수는 구면좌표를 3차원 직교 좌표계로 변환하는 테스트 함수이다..
 */

void CalcAir::LatLonToCartesianTest(double lat_angle, double lon_angle)
{
	QPointF p1 = QPointF(lat_angle, lon_angle);
	double x, y, z;

	//3차와 직교 좌표로 변환
	x = EarthRadius * cos(lat_angle) * cos(lon_angle);
	y = EarthRadius * cos(lat_angle) * sin(lon_angle);
	z = EarthRadius * sin(lat_angle);

	qDebug()<<"[TEST]LATLON : "<<p1<<" Cartesian X : "<<QString().setNum(x, 'f', 6)<<", Y : "<<QString().setNum(y, 'f', 6)
				<<", Z : "<<QString().setNum(z, 'f', 6);
