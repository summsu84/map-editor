...
LatLonConvert * LatLonConvert::pinstance = NULL;

LatLonConvert::LatLonConvert() {
	// TODO Auto-generated constructor stub
	qDebug()<<"LatLonConvert::LatLonConvert is called..";
	wonJum = QPointF(35.0, 127.0);
	screenRect = QRect(0, 0, 600, 600);
//	meterPerPixel = 2000;
	meterPerPixel = EarthRadius / 600;
	meterPerPixel_2 = EarthRadius / 5000;
	cc.setCenter(screenRect.width()/2, screenRect.height()/2);
	qDebug()<<"=================meterPerPixel_2 : "<<meterPerPixel_2;
}
...
double LatLonConvert::mppToDegree()
{
	...
	//화면의 크기를 바탕으로 1픽셀에 대한 degree를 가져온다..

	//qDebug()<<"[LatLonConvert - mppToDegree] degree : "<<degree;
	double dy = 10.0;
	if (degree < 1) {
		dy = 0.1;
	}else if(degree < 4) {
...
QPoint  LatLonConvert::latlonToScreen(const QPointF& decimal_latlon )
{
	...
	qDebug()<<"[LatLonConvert - latlonToScreen] point : "<<QPoint(y1,z1);
	return QPoint(y1,z1);
}

/*
 * LatLon(위경도) 좌표를 X,Y 좌표로 변환한다..
 */
QPointF  LatLonConvert::latlonToScreen2(const QPointF& decimal_latlon )
{

	QPointF d = QPointF(decimal_latlon.x(), decimal_latlon.y() - wonJum.y());
	qDebug()<<"[LatLonConvert - latlonToScreen] decimal_latlon ; "<<QString::number(decimal_latlon.x(), 'f', 6)<<", "<<QString::number(decimal_latlon.y(), 'f', 6)<<", d : "<<d<<", mpp : "<<meterPerPixel<<"wonjum : "<<wonJum;
    ...
    //  vp distence
	double r1  = sqrt(xyz.z * xyz.z + xyz.y * xyz.y);		//pitagoras therom
	...
	//std::cout << "latlonToScreen y1 : " << y1 << ", z1 : " << z1 << "\n";
	//qDebug()<<"PixelUnit : " << pixelUnit << ", r1 : " << r1 << ", r2 : " << r2 << ", bojung_r : " << bojung_r << ", dy : " << y << ", dz " << z;
	//qDebug()<<"latlonToScreen y1 : " << y1 << ", z1 : " << z1;

	//qDebug()<<
	qDebug() << "latlonToScreen y1 : " << y1 << ", z1 : " << z1 << ", Wonjum : "<<wonJum
			<<",meterPerPixel_2 : "<<meterPerPixel_2<<" xyz - x : "<<xyz.x<<", xyz -y : "<<xyz.y<<", xyz- z : "<<xyz.z;
...
QPointF LatLonConvert::screenToDecimal(int screenX, int screenY)
{
	...
	//qDebug()<<"[LatLonConvert- STD] sx : "<<screenX<<", sy : "<<screenY<<", P : "<<p<<"meterPerPixel : "<<meterPerPixel;
	//qDebug()<<"[STD Point ] "<<QPointF(p.x(), p.y() + wonJum.y())<<" sx : "<<screenX<<", sy : "<<screenY<<", P : "<<p<<"meterPerPixel : "<<meterPerPixel;
...
//added 20130701
/*QString LatLonConvert::screenToLatLon(const QPointF& screenXY)
{
	QPointF p = screenToDecimal(screenXY);
	return ca.toDMS(p);
}*/
//added 20130701
/*QPointF LatLonConvert::screenToDecimal(const QPointF& screenXY)
{
	return screenToDecimal(screenXY.x(), screenXY.y());
}
//added 20130701
...
QString LatLonConvert::decimalLatToString(double lat)
{

	QString str_lat = QString::number(lat, 'f', 6);
...
QString LatLonConvert::decimalLonToString(double lon)
{
	QString str_lon = QString::number(lon, 'f', 6);
...
//두 포인트 (X, Y) 사이의 거리를 구한다.

double LatLonConvert::distanceXY(QPoint p1, QPoint p2)
{
